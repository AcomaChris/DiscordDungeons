...
// --- enrich-object-defs.js ---
// Auto-enriches object definitions with WFC edges, parts, and nodes.
// Idempotent: skips fields already populated.
//
// Usage: node scripts/enrich-object-defs.js <tileset-name>
//        node scripts/enrich-object-defs.js --all
//        node scripts/enrich-object-defs.js <tileset-name> --dry-run
...
const DEFS_DIR = join(ROOT, 'client/public/object-defs');

const TILE_SIZE = 16;

// --- Tag Helpers ---

function hasTags(obj, ...tags) {
...
// --- 2a. WFC Edge Refinement ---
// AGENT: Socket names must match _sockets.json.

// Default bootstrap edges (used to detect if edges were manually set)
...
function enrichEdges(obj) {
  if (!obj.wfc) return false;
  if (!edgesAreBootstrapDefault(obj)) return false; // manually set
  ...
  // 1×1 decorations — keep open_floor (they sit on surfaces, not on floor)
  if (is1x1 && obj.category === 'decoration') {
    return false; // leave unchanged
  }

  // Freestanding furniture (tables, benches, chairs, stools, sofas, barrels)
...
function enrichParts(obj) {
  if (obj.parts) return false; // already set
  if (obj.grid.cols < 3) return false; // too narrow for stretch
  if (hasNullsInGrid(obj)) return false; // sparse grids don't stretch cleanly

  // Only apply to known stretchable types
  const isStretchable = hasTags(obj, 'table', 'bench', 'counter', 'shelf', 'bookshelf');
  if (!isStretchable) return false;

  // Skip round objects and complex shapes
  if (hasTags(obj, 'round', 'sofa', 'stair', 'corner', 'water', 'pool')) return false;

  const { cols, rows } = obj.grid;

  // Build layout: first col = left_end, last col = right_end, inner = middle
  const layout = [];
...
function enrichNodes(obj) {
  if (obj.nodes && obj.nodes.length > 0) return false; // already set
  ...
  if (hasTags(obj, 'table') && obj.category === 'furniture') {
    nodes.push({
      ...
      type: 'item_placement',
      x: centerX,
      y: Math.floor(pixelH / 3),
      elevation: 1,
    });
    // Sit nodes offset below the table (south side)
    if (obj.grid.cols >= 2) {
      nodes.push({
        id: 'sit_south_left',
        type: 'sit',
      ...
      nodes.push({
        id: 'sit_south_right',
        type: 'sit',
        x: pixelW - Math.floor(TILE_SIZE / 2),
        y: pixelH + 8,
        elevation: 0,
        facing: 'up',
      });
  ...
  // Bookshelves — interact node at south face
...
function enrichTileset(tilesetName, dryRun) {
  const filePath = join(DEFS_DIR, `${tilesetName}.objects.json`);
  let data;
  try {
  ...
  const total = Object.keys(data.objects).length;
  console.log(`${tilesetName}: ${total} objects`);
  console.log(`  Edges enriched: ${edgeChanges}`);
  console.log(`  Parts added: ${partsChanges}`);
  console.log(`  Nodes added: ${nodeChanges}`);

  if (!dryRun && (edgeChanges + partsChanges + nodeChanges) > 0) {
...
if (tilesetArg === '--all' || args.includes('--all')) {
  const files = readdirSync(DEFS_DIR).filter(f => f.endsWith('.objects.json'));
  for (const file of files) {
    const name = file.replace('.objects.json', '');
    enrichTileset(name, dryRun);
  }
} else if (tilesetArg) {
  enrichTileset(tilesetArg, dryRun);
} else {
  console.error('Usage: node scripts/enrich-object-defs.js <tileset-name|--all> [--dry-run]');
  process.exit(1);
}
